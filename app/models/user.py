...
class User(db.Model):
    ...
    item = db.relationship("Item", cascade="all,delete", backref="user", lazy=True)
    category = db.relationship(
        "Category", cascade="all,delete", backref="user", lazy=True
    )
    bank = db.relationship("Bank", cascade="all,delete", backref="user", lazy=True)
